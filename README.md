...
An event-driven runtime for building reactive systems with a simple **Event → Handler → Event** flow.
...
**Event → Handler → Event.** Simple, direct flow with flexible state management.

## What Seesaw Is / Is Not

Seesaw **is**:

> An event-driven runtime where events flow through handlers, which return new events.
> State lives in events or shared dependencies. Clean entry points via closures.
...
- **Stateless Engine**: Reusable runtime, dependencies and store provided at creation
...
- **Procedural Macros**: Optional `#[handler]` attributes for concise syntax (v0.10+)
- **Event Handlers**: React to events, perform IO, return new events
- **Flexible State**: Event-threaded, shared dependencies, or implicit state patterns
- **DLQ Terminal Mapping**: Map exhausted retries to terminal events for explicit failure handling (v0.10.2+)
- **Event Batching**: Emit and accumulate multiple events for bulk operations (v0.10.0+)
- **Pipedream Integration**: Stream composition and bidirectional piping
- **Direct Dispatch**: `engine.dispatch(event)` for clean execution
...
```rust
use seesaw_core::{handler, Engine};
...
async fn main() -> Result<()> {
    ...
    let engine = Engine::new(deps, store)
        // Handler that reacts to Placed and returns Shipped
        .with_handler(
        ...
        // Handler that reacts to Shipped and returns Delivered
        .with_handler(
            handler::on::<OrderEvent>()
                .extract(|e| match e {
    ...
    // Dispatch event directly - engine handles everything
...
Use `#[handlers]` and `#[reducers]` to automatically generate registration functions:
...
handler::on::<OrderEvent>().then(|event, ctx| async move {
    ctx.deps().logger.log(&event);
    Ok(Emit::None)  // No event dispatched
})

// Observe ALL events
handler::on_any().then(|event, ctx| async move {
...
handler::on::<PaymentRequested>()
    .name("charge_payment")               // Custom ID for tracing
...
- **Queued**: Triggered by `.background()`, `.delay()`, `.timeout()`, `.retry() > 1`, or `.priority()`
...
**Alternative: `HandlerError` events**

For convenience, use `?` in effects and handle `HandlerError` events:
...
handler::on::<ImportEvent>()
    .accumulate()  // Accumulates events with same batch_id
...
- `.accumulate()` accumulates all events with matching `batch_id`
    ...
    .with_handler(
        handler::on::<OrderEvent>()
            ...
            .then(|order_id, ctx| async move {
                ...
                ctx.deps().dispatch(order_id).await?;
...
- `.dispatch(async_closure).await` — Async version of `run()`, waits for completion
