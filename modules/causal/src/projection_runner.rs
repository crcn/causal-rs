//! Runners that drive [`Materializer`](crate::Materializer) and
...
//!     `materialize` returned `Ok` for that specific fact).
...
//! `materialize` errors stop cursor advance and propagate up; the
...
// ─────────────────────────────────────────────────────────────────────
// ProjectionRunner — drives a Materializer
// ─────────────────────────────────────────────────────────────────────

pub struct ProjectionRunner<M: Materializer> {
    materializer: M,
    consumer_id:  String,
...
impl<M: Materializer> ProjectionRunner<M>
where
    M::Fact: DeserializeOwned,
{
    ...
    ///
    /// Per C2: cursor advances per-fact, only after `materialize`
    ...
    pub async fn step(&self, batch: usize) -> Result<StepOutcome> {
        ...
        for event in events {
            ...
            // materialize doesn't double-apply on retry. Mirrors the
            ...
            // cursor but don't trigger materialize — the consumer
            ...
            match self.materializer.materialize(&fact, ctx).await {
                Ok(()) => {
    ...
    /// always ZERO) and service-level materializers (no aggregators)
...
mod tests {
    ...
    struct CollectingMaterializer {
        seen: Arc<parking_lot::Mutex<Vec<Uuid>>>,
    }

    #[async_trait]
    impl Materializer for CollectingMaterializer {
        type Fact = Recorded;
    ...
    /// Materializer that fails on the Nth call (1-indexed).
    struct FailsOnNth {
        n:     usize,
    ...
    #[async_trait]
    impl Materializer for FailsOnNth {
        type Fact = Recorded;
    ...
    #[tokio::test]
    async fn materializer_ctx_carries_persisted_event_correlation_id() {
        // The Materializer's ctx is built from the persisted event,
        // not regenerated. Tracing through a fold: emit a fact with
        // explicit correlation_id, run the materializer, assert the
        ...
        #[async_trait]
        impl Materializer for CorrelationCapture {
            type Fact = Recorded;
            async fn materialize(
    ...
    #[tokio::test]
    async fn materializer_runner_advances_cursor_per_fact() {
        let store = Arc::new(MemoryStore::new());
        let _ids = append_n(&store, 3).await;
        let seen = Arc::new(parking_lot::Mutex::new(Vec::new()));
        let runner = ProjectionRunner::new(
            CollectingMaterializer { seen: seen.clone() },
            "m1",
            store.clone() as Arc<dyn EventLogBackend>,
            store.clone() as Arc<dyn CheckpointStore>,
        );
    ...
    #[tokio::test]
    async fn materializer_runner_does_not_advance_past_failure() {
        let store = Arc::new(MemoryStore::new());
        ...
        assert_eq!(cursor, events[1].position,
                   "cursor at fact[1] (after two successful materializes), \
                    not at fact[2] (where materialize errored)");
    }
    ...
    async fn dep_fence_holds_when_dep_behind() {
        // Build a Materializer with a hard-coded dep on "upstream".
        struct DepM {
            seen: Arc<parking_lot::Mutex<Vec<Uuid>>>,
        }
        #[async_trait]
        impl Materializer for DepM {
            type Fact = Recorded;
    ...
    async fn non_matching_event_type_is_skipped_and_cursor_advances() {
        ...
        // materializer's `Fact::type_prefix()`. Runner should advance
        // past it without calling materialize.
        let store = Arc::new(MemoryStore::new());
    ...
    async fn fact_without_occurred_at_falls_back_to_event_created_at() {
        ...
        #[async_trait]
        impl Materializer for CaptureNow {
