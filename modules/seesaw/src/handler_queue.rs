//! Handler work-distribution queue trait.
//!
//! The [`HandlerQueue`] trait abstracts handler intent persistence, dequeue,
...
use crate::types::{
    HandlerResolution, IntentCommit, JournalEntry, QueueStatus, QueuedHandler,
};

/// Work-distribution queue for handler executions.
///
...
/// `enqueue()` which atomically persists handler intents AND advances
/// the checkpoint.
#[async_trait]
pub trait HandlerQueue: Send + Sync {
    /// Atomically enqueue handler intents and advance the checkpoint.
    ///
    ...
    ///
    /// Used by the settle loop to sleep until the next handler becomes
    ...
    ///
    /// Single exit point for every claimed handler:
    ...
    /// Reclaim handlers stuck in "running" state beyond their timeout.
