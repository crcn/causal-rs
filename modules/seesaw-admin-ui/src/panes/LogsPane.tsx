    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-3 py-2 border-b border-border flex items-center gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-1 text-[11px]">
          ...
          {logsFilter.runId && (
            <button
              onClick={() => dispatch({ type: "ui/logs_filter_changed", payload: { scope: "run" } })}
              className={`px-2 py-0.5 rounded ${logsFilter.scope === "run" ? "bg-accent text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              This run
            </button>
          )}
