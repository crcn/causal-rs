...
/// Generic admin query resolvers for seesaw event tables.
///
...
///
/// ```ignore
/// use seesaw_admin::{SeesawAdminQuery, DefaultEventDisplay};
///
/// let admin = SeesawAdminQuery::new(DefaultEventDisplay);
...
impl<D: EventDisplay> SeesawAdminQuery<D> {
...
pub struct SeesawAdminSubscription<D: EventDisplay> {
    display: Arc<D>,
}

impl<D: EventDisplay> SeesawAdminSubscription<D> {
    pub fn new(display: D) -> Self {
        Self {
            display: Arc::new(display),
        }
...
impl<D: EventDisplay + 'static> SeesawAdminSubscription<D> {
