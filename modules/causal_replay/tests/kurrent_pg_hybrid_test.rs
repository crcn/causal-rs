//! End-to-end smoke test for the Kurrent-log + Postgres-reactor-outbox
//! hybrid deployment.
//!
//! This is the configuration `rootsignal` plans to ship: KurrentDB
//! as the durable event log (per-aggregate streams, `$all`,
//! server-side projections) and Postgres as the reactor outbox +
//! cursor store (atomic batch commit, SKIP LOCKED claims). The
//! roadmap calls this "Option B (hybrid)" — see
...
use causal::{
    CheckpointStore, EngineBuilder, EventLogBackend, Event, ReactorOutbox,
};
...
async fn engine_appends_to_kurrent_through_pg_outbox() -> Result<()> {
    let kurrent = KurrentEventLogBackend::connect(&kurrent_url())?;
    let pg_pool = connect_pg().await;
    let pg = Arc::new(PgReactorOutbox::new(pg_pool.clone()));
    ...
    assert_eq!(stream[0].aggregate_type.as_deref(), Some("order"));
    assert_eq!(stream[0].aggregate_id, Some(order_id));
