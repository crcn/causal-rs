...
# Backend Simplification: Causal + Restate
...
Restate (durability, retries, state)
  └── Causal (routing, composition, DSL, settle loop)
...
- **Delete `causal-postgres`**: Its durable queue store and InsightStore impl are both replaced by Restate.
...
| Crate/Module | Reason |
|---|---|
| `crates/causal-postgres/` | Queue store → Restate. InsightStore → dropped. |
| `crates/causal-memory/` | InsightStore impl → dropped. MemoryEventStore → folded into core. |
| `crates/causal-insight/` | Restate dashboard replaces it. |
...
1. Move `MemoryEventStore` from `causal-memory` into `causal/src/es/`
2. Delete `crates/causal-postgres/`, `crates/causal-memory/`, `crates/causal-insight/`
