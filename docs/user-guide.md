...
let effects = on! {
    // Critical payment - long timeout, more retries
    #[effect(id = "charge_payment", timeout = 60, retry = 5, priority = 1)]
    OrderPlaced { order_id, amount_cents, .. } => |ctx| async move {
        let charge = ctx.deps().stripe.charge(
            amount_cents,
            idempotency_key: &ctx.idempotency_key  // Framework provides this
        ).await?;
    ...
    #[effect(id = "send_confirmation")]
    OrderPlaced { customer_email, .. } => |ctx| async move {
        ctx.deps().mailer.send_confirmation(&customer_email).await?;
        Ok(())  // Observer - no event returned
    },

    // Request approval after 2 days
    #[effect(id = "request_approval", delayed = 2days)]
    OrderPlaced { order_id, .. } => |ctx| async move {
        ctx.deps().mailer.send_approval_request(order_id).await?;
        Ok(ApprovalRequested { order_id })
    },

    // Send reminder after 7 more days
    #[effect(id = "send_reminder", delayed = 7days)]
...
async fn main() -> Result<()> {
    ...
    for effect in effects { engine = engine.with_effect(effect); }
...
```rust
#[effect(id = "charge_payment")]  // ← ID required at compile-time
...
#[effect(id = "send_email", timeout = 10, retry = 3)]

// Logs: no retries needed
#[effect(id = "log", timeout = 5, retry = 1)]
```
...
#[effect(id = "system_alert", priority = 1)]  // Higher priority
SystemAlert { .. } => |ctx| async move { ... }

// Normal events
#[effect(id = "user_signup", priority = 10)]  // Default priority
...
OrderPlaced { .. } => {
    #[effect(id = "charge")] => charge_payment,
    #[effect(id = "confirm")] => send_confirmation,
    #[effect(id = "notify_ops")] => notify_ops_team,
}
