...
let charge_effect = handler::on::<OrderPlaced>()
...
let confirm_effect = handler::on::<OrderPlaced>()
    .then(|event, ctx| async move {
...
let approval_effect = handler::on::<OrderPlaced>()
    .then(|event, ctx| async move {
...
let reminder_effect = handler::on::<ApprovalRequested>()
    .then(|event, ctx| async move {
...
let complete_effect = handler::on::<ApprovalReceived>()
    .then(|event, ctx| async move {
...
let charge_effect = handler::on::<OrderPlaced>()
    .name("charge_payment")                    // ← Required (compile-time enforced)
...
let approval_effect = handler::on::<OrderPlaced>()
    ...
    .delay(Duration::from_days(2))         // ← Execute in 2 days!
...
let reminder_effect = handler::on::<ApprovalRequested>()
    .name("send_reminder")
    .delay(Duration::from_days(7))         // ← Execute in 7 days!
    .retry(3)
    .then(|event, ctx| async move {
...
## 5. Effects with Procedural Macros

### v0.7 (Stateless)

```rust
// No attribute macro support in v0.7
```
...
mod order_effects {
    ...
    #[handler(on = OrderPlaced, id = "charge_payment", priority = 1, timeout_secs = 60, retry = 5)]
    ...
    #[handler(on = [OrderEvent::Placed, OrderEvent::Updated], extract(order_id), id = "sync_order")]
    async fn sync_order(order_id: Uuid, ctx: HandlerContext<Deps>) -> anyhow::Result<()> {
...
| **Delayed effects** | ❌ Not possible | `.delay(2days)` | Multi-day workflows |
...
3. **Delays are built-in** - `.delay(7days)` just works
