...
impl EventLogBackend for LaggingLog {
    ...
    async fn read_stream(
        ...
        if aggregate_type == CONTROL_SUBJECT {
            return Err(anyhow!("audit fault: storage blip during fence rehydration"));
        }
        EventLogBackend::read_stream(self.inner.as_ref(), aggregate_type, aggregate_id, after).await
    }
    async fn latest_position(&self) -> Result<LogCursor> {
...
async fn run_scenario(seal_empty: bool) -> (usize, usize, Vec<bool>, Vec<String>) {
    ...
    // ── Incarnation 2: rehydration blip + lagging scan ───────────────
    ...
    .build()
    .await
    .expect("engine2 build (fence rehydration failure is swallowed)");
...
// ── Test 1: A6 elision ON — asserts the library promise; FAILS if the
//    defect is real. ─────────────────────────────────────────────────

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn elided_empty_cancel_decision_must_not_re_decide_on_redelivery() {
    let (outputs, calls, flags, sealed_after_inc1) = run_scenario(false).await;

    // Elision means nothing was sealed for the first (empty) decision.
    assert!(
        sealed_after_inc1.is_empty(),
        "with seal_empty_decisions(false) the empty decision seals nothing; got {sealed_after_inc1:?}"
    );
